...
import groundcrew.llm.ollamaapi as oai

with open('ollama_key_file', 'r') as f:
    ollama_key = f.read().strip()

client = oai.get_ollamaai_client(ollama_key)
...
def start_chat(model: str, client: ollama.Client) -> Callable:
    ...
    of the ollama API, e.g., `tools`, `temperature`, `seed`, etc."""
